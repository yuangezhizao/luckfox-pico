...
| 组成 | 字节数 | 算式 |
| --- | ---: | --- |
| `__symbols__` 节点（365 条）、`-@` 多分配的 234 个 `phandle`，以及 150 个因带 label 而免于 `/omit-if-no-ref/` 删除的 pinctrl 节点 | 32,357 | 76,708 − 44,351 |
...
| 项目 | 修复前（自编译） | 修复后（预期） | 官方 260604 |
| --- | ---: | ---: | ---: |
| DTB 体积 | 76,708 B | 41,740 B | 41,740 B |
| kernel 偏移 | `0x13400` | `0xac00` | `0xac00` |
| resource 体积 | 114,688 B | 79,872 B | 79,872 B |
| `boot.img` 总体积 | 3,862,528 B | 3,792,896 B | 3,793,408 B |

前三行不是估算：DTB 已用内核自带 dtc 离线重编验证为 41,740 B 且与官方逐字节相同（§6.2），`kernel` 偏移与 `resource` 体积则由 §3.3 那套已在三份镜像上验证的块对齐规律直接算出。第四行按同一规律推得 `2048 + 41740 → kernel@44032 + 3667920 → resource@3712000 + 79872 → 尾部补 1024`，即 3,792,896 B；本行仍属预推，需重编后核对实际产物。

预期值与官方相差 512 字节：内核二进制本身差 456 字节（版本串压缩后长度不同，属构建元数据，无需也无法消除），经 512 字节块对齐后放大为整块之差。
