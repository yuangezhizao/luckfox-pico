...
Cursor 文档把 Dockerfile `USER ubuntu`（及可选的 `WORKDIR /home/ubuntu`）标为良好实践；schema 另提供可选 `user`。两条路径都能把会话用户变成 `ubuntu`（实测见 plan「最终验证证据」）。本仓 `.cursor/Dockerfile*` 虽为 Cloud Agent 专用，仍以镜像 `USER` 为唯一来源，避免身份只写在 Cursor JSON 里。
