...
- 镜像已含 `git`（Dockerfile 的 apt 安装块已装 `git`），便于容器内常规 git 操作；`checkout` 用常规模式即可——仓库根目录无 `.gitmodules`、无真子模块，编译所需的 lvgl 等均为普通 vendored 目录；仓库另有一个与本 3 个 Buildroot 组合无关的孤立 gitlink（`sysdrv/tools/board/ubuntu`，mode 160000、无 `.gitmodules` 映射、指向 Ubuntu rootfs），常规 checkout 对它仅留空目录、不影响编译，故无需 `submodules: recursive`（YAGNI，将来真加 submodule 再补）。**⚠️ CI 首测实测**：`actions/checkout` 的认证清理步会对该孤立 gitlink 执行 `git submodule foreach`，因无 `.gitmodules` 登记 URL 而报 `fatal: No url found for submodule path 'sysdrv/tools/board/ubuntu' in .gitmodules` → git 退出码 128，产生**一条良性 `warning` annotation**（每 job 一条，首测 4 条）——**非致命、不阻断**，三组合 run 全绿、artifact 正常；`submodules: false`（本就默认）也挡不住此认证清理步。根治需移除该 gitlink（`git rm --cached sysdrv/tools/board/ubuntu`）或补 `.gitmodules` 登记其 URL，但属**上游 SDK 数据改动、超本 CI 范围**，故不在本 workflow 处理（保留为可选的独立仓库清理任务）。
