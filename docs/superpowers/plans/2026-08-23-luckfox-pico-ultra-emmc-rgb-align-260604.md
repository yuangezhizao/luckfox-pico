...
**执行状态：** Task 1–4 已全部完成；下方步骤与末尾验证证据已按实际执行结果回填。
...
- 提交信息用 Conventional + gitmoji（`git cz` 风格），subject 单一主题；spec 与 plan 两份文档留到 PR 的**最后一次提交**。本仓提交由环境自动附加一条 `Co-authored-by:` trailer，`dev` 上既有 11 个非合并提交均如此，属既定惯例，**不要为去掉它而 amend**。
- 判断工作区是否干净时一律按路径限定检查，不要用不带路径的 `git status` / `git diff` 下结论——工作区可能存在与本计划无关的既有改动或未跟踪文件。
...
- [x] **Step 1: 构建内核自带的 dtc（含 Rockchip 裁剪检查），全程在 `/tmp`**
...
- [x] **Step 2: 写下离线检查命令，并在改动前运行一次，确认缺陷可复现**
...
- [x] **Step 4: 再次运行离线检查，确认与官方 260604 逐字节相同**
...
**注意**：工作区可能存在与本任务无关的既有改动或未跟踪文件，**不要动、也不要提交**。因此只按上面的路径限定命令检查，不要用不带路径的 `git diff --stat` 判断「工作区只有一处改动」。若 `project/app/wifi_app/` 下出现 `hostapd` / `hostapd_cli` / `librkwifibt.so` 被改写，用 `git checkout -- <file>` 恢复后再提交。
...
- [x] **Step 5: 校验新 `boot.img` 的 DTB 与整体布局**
...
- [x] **Step 2: 等待启动，采集出厂态（720×720）指标**
...
期望：打印 `***Switch the RGB screen resolution to 480 x 480.***`；重启后 `modetest` 报 `#0 480x480 59.94 … 16500`。若仍显示 720×720，再 `adb reboot` 一次——boot 分区 DTB 改写偶尔需要第二次重启才稳定。

- [x] **Step 4: 按 spec §8 判据逐项采集**
...
## 附录：跨平台复现离线核查（含 Cloud Agent 环境）

上面 Task 1、Task 3、Task 4 各步骤中的命令是**当时在 macOS 本机执行的原始记录**，其中的绝对路径（`/Users/yuangezhizao/…`）与 BSD 语法（`stat -f%z`、`shasum -a 256`）予以保留，以便日后核对当时到底跑了什么。本附录另给出一份**双平台通用**的等价实现，用于在 Linux、Cloud Agent 或任何其他机器上重放同一核查。

前置条件：A.1 依赖 `git rev-parse` 推导仓库根，须在仓库工作树内执行（任意子目录均可）；A.2 复用 A.1 构建出的 dtc，须先跑过 A.1。两段脚本的中间产物全部落在 `/tmp/dtbcheck`，不触碰工作区。

### A.1 单板型离线核查（等价于 Task 1 Step 2 / Step 4）

路径由 `git rev-parse` 推导，体积用 `wc -c`，哈希按平台自动选择 `sha256sum` 或 `shasum`，因此无需改动即可在 Linux 与 macOS 上运行。脚本按 defconfig 里 `CONFIG_DTC_SYMBOLS` 的**实际取值**决定是否给 dtc 传 `-@`，因此它同时充当回归检查。

```bash
#!/usr/bin/env bash
set -euo pipefail
REPO=$(git rev-parse --show-toplevel)
K="$REPO/sysdrv/source/kernel"
DEFCONFIG="$K/arch/arm/configs/luckfox_rv1106_linux_defconfig"
DTS="${1:-$K/arch/arm/boot/dts/rv1106g-luckfox-pico-ultra.dts}"
WORK=/tmp/dtbcheck
...
# 编内核自带 dtc：上游 dtc 不认识 Rockchip 私有的 -Wnode_disabled / -Wnode_empty
if [ ! -x "$WORK/dtc/dtc-kernel" ]; then
  rm -rf "$WORK"; mkdir -p "$WORK"; cp -R "$K/scripts/dtc" "$WORK/"
  ( cd "$WORK/dtc"
    bison -o dtc-parser.tab.c -d dtc-parser.y
    flex -o dtc-lexer.lex.c dtc-lexer.l
    echo '#define DTC_VERSION "DTC 1.6.0-local"' > version_gen.h
...
if grep -q '^CONFIG_DTC_SYMBOLS=y' "$DEFCONFIG"; then SYM=(-@); else SYM=(); fi
echo "defconfig: CONFIG_DTC_SYMBOLS $([ ${#SYM[@]} -gt 0 ] && echo 开启 || echo 未设置)"
...
echo "DTB 体积 : $(wc -c < out.dtb) B"
echo "DTB SHA  : $(sha256 out.dtb)"
dtc -I dtb -O dts out.dtb 2>/dev/null > out.dts
printf '__symbols__ : %s\n' "$(grep -q '__symbols__' out.dts && echo 有 || echo 无)"
python3 - out.dts <<'PY'
import re, sys
s = open(sys.argv[1]).read()
m = re.search(r'^[ \t]*(?:[\w-]+[ \t]*:[ \t]*)?backlight \{(.*?)^[ \t]*\};', s, re.S | re.M)
...
修复后（`CONFIG_DTC_SYMBOLS` 未设置）期望输出 `41740 B` / `60e6928362602b44f6884e9e5f0b20996f23574db4d29bfed378809670e15cfa` / `__symbols__ 无` / `backlight phandle 无`；把该选项改回 `=y` 再跑一次，应得 `76708 B` / `70da0b6a1f98acdc7e64e2d986103323e838c2809086e035466dce2142c6d1f5` / `__symbols__ 有` / `backlight phandle 0x164`。

### A.2 跨板型裁剪核查（§6.4 影响半径的复现手段）

改动 dtc 全局开关的影响面无法靠读代码穷举，但可以对每个板型编译两组 DTB、比较体积并检查关键节点是否被裁剪，几秒内答完。这是 spec §6.4 那张表的生成方式，`NODE` 换成别的节点名即可复用于其它同类改动。
...
REPO=$(git rev-parse --show-toplevel)
K="$REPO/sysdrv/source/kernel"
DTC=/tmp/dtbcheck/dtc/dtc-kernel      # 由 A.1 构建
NODE="${1:-rgb}"
# 下面 dtc 调用的 stderr 被丢弃，缺少该二进制时会只打印表头就静默退出 127，故先显式检查
[ -x "$DTC" ] || { echo "找不到 $DTC：请先运行 A.1 构建内核自带 dtc" >&2; exit 1; }
INC=(-i"$K/arch/arm/boot/dts/" -i"$K/scripts/dtc/include-prefixes")
printf '%-32s %9s %9s  %s\n' 板型DTS 带-@ 不带-@ "$NODE 节点"
for f in "$K"/arch/arm/boot/dts/rv110[36]g-luckfox-pico*.dts; do
  b=$(basename "$f" .dts)
  gcc -E -nostdinc -I"$K/scripts/dtc/include-prefixes" -I"$K/arch/arm/boot/dts" -I"$K/include" \
    -undef -D__DTS__ -x assembler-with-cpp -o /tmp/sw.dts "$f"
  "$DTC" -O dtb -o /tmp/sw-on.dtb  -b 0 "${INC[@]}" -@ -Wnode_disabled -Wnode_empty /tmp/sw.dts 2>/dev/null
  "$DTC" -O dtb -o /tmp/sw-off.dtb -b 0 "${INC[@]}"    -Wnode_disabled -Wnode_empty /tmp/sw.dts 2>/dev/null
  keep=$(dtc -I dtb -O dts /tmp/sw-off.dtb 2>/dev/null | grep -c "^[[:space:]]*${NODE} {" || true)
  printf '%-32s %9s %9s  %s\n' "$b" "$(wc -c < /tmp/sw-on.dtb)" "$(wc -c < /tmp/sw-off.dtb)" \
    "$([ "$keep" -gt 0 ] && echo 保留 || echo 被裁剪)"
done
```

该 glob 会额外匹配到 `rv1106g-luckfox-pico-pro-max-fastboot` 与 `rv1106g-luckfox-pico-ultra-fastboot` 两个 DTS，它们由 FASTBOOT 组合使用、走 `luckfox_rv1106_linux_tb_defconfig`，而该 defconfig 本就没有 `CONFIG_DTC_SYMBOLS`，因此不受本改动影响，比对时可忽略。

### A.3 在 Cloud Agent 环境中 debug

Cloud Agent 环境已预装 `gcc` / `bison` / `flex` / `dtc` / `python3`，A.1 与 A.2 可直接运行，无需额外安装。使用时注意四点：

- **官方对照固件不在环境内**。spec 中一切「与官方固件逐字节相同」「oem / rootfs 清单逐条一致」之类断言依赖 `Luckfox_Pico_Ultra_W_EMMC_250607` 与 `Luckfox_Pico_Ultra_eMMC_260604` 两份镜像，它们只在本机。云端只能验证「重编产物是否等于文档记载的体积与 SHA256」，这已足以判定 `CONFIG_DTC_SYMBOLS` 的效果，但无法独立确认与官方的一致性。
- **无实体板**。所有板级判据（`bl_power`、`iomux 3 27`、`/sys/kernel/debug/pwm`、`modetest`、屏幕目视）都必须在接了板子的机器上做。
- **CI artifact 可用于交叉验证**，见 A.4。
- **Cloud Agent 的 `gh` 是只读的**。可以 `gh pr view` / `gh run view` / `gh run download`，但 `gh pr edit` 会报 `Resource not accessible by integration`；修改 PR 标题或正文需要另外的可写通道。

### A.4 用 CI artifact 交叉验证 boot.img

`gh run download` 默认从当前目录的 git remote 推断仓库，在仓库外执行会直接报 `fatal: not a git repository`，因此务必显式带上 `-R`，并用 `-D` 指定落盘目录：

```bash
gh run download 32616072092 -R yuangezhizao/luckfox-pico \
  -n luckfox-pico-firmware_ultra-emmc_on-ubuntu-24.04 -D /tmp/ci-ultra
```

`-D` 指定的目录必须为空或不存在：`gh run download` 不覆盖已有文件，重跑时会报 `error extracting …: file exists`，所以重复取回前先 `rm -rf /tmp/ci-ultra`。

Ultra 组合的 artifact zip 为 `180 MiB`、解压后 `912 MiB`（13 个文件），下载加解压的峰值占用约 `1.1 GiB`，预留 `1.5 GB` 即可。取回后用下面这段解析 `boot.img` 的 FIT 头，与 A.1 的离线重编结果互为印证：

```bash
python3 - /tmp/ci-ultra/output/image/boot.img /tmp/dtbcheck/out.dtb <<'PY'
import hashlib, re, struct, subprocess, sys
boot, ref = sys.argv[1], sys.argv[2]
d = open(boot, 'rb').read()
open('/tmp/fit-hdr.dtb', 'wb').write(d[:struct.unpack('>I', d[4:8])[0]])
hdr = subprocess.check_output(['dtc', '-I', 'dtb', '-O', 'dts', '/tmp/fit-hdr.dtb'],
                              stderr=subprocess.DEVNULL).decode()
def num(sec, key):
    m = re.search(rf'{sec} \{{[^}}]*{key} = <(0x[0-9a-f]+)>', hdr, re.S)
    return int(m.group(1), 16) if m else None
fo, fs = num('fdt', 'data-position'), num('fdt', 'data-size')
dtb = d[fo:fo + fs]
print(f'boot.img 总体积 : {len(d)} B')
print(f'FIT fdt 体积    : {fs} B')
print(f'FIT fdt SHA256  : {hashlib.sha256(dtb).hexdigest()}')
print(f'kernel 偏移     : {hex(num("kernel", "data-position"))}')
print(f'resource 体积   : {num("resource", "data-size")} B')
try:
    print(f'与 {ref} 逐字节相同 : {"是" if dtb == open(ref, "rb").read() else "否"}')
except FileNotFoundError:
    print(f'与 {ref} 比对      : 跳过（文件不存在，请先运行 A.1）')
PY
```

run `32616072092` 的期望输出为 `3793408 B` / `41740 B` / `60e6928362602b44f6884e9e5f0b20996f23574db4d29bfed378809670e15cfa` / `0xac00` / `79872 B`，且与 A.1 产物**逐字节相同**。

本附录全部代码块均置于顶层缩进：块内含 heredoc，若嵌进列表项，从原始文件复制会因终止符带缩进而失效。
