...
**Architecture:** 身份只来自两条 Dockerfile 末尾的 `USER ubuntu`（备选官方 FROM 无账户时先 `id -u ubuntu || useradd`）。`.cursor/environment.json` 不声明 `user`；`install` / `start` 为 `sudo -n -E bash .cursor/*.sh`。脚本正文保持 root 语义。grilling 写到 `$HOME/.cursor/skills/` 后 `chown -R ubuntu:ubuntu`。PID 1 / `tailscaled` / sshd 仍为 root。活动 Dockerfile 同时是固件 CI 镜像；`build-firmware.container.options` 为 `--user 0`。
...
- 本 PR 相对 `origin/dev` 三个提交：实现（仅 `.cursor/Dockerfile*`、`environment.json`、`install.sh`）在前，固件 CI workflow 居中，`AGENTS.md` 与全部 superpowers 文档在最后。改实现合进第一个提交并保留其 AuthorDate `2026-09-11 01:30:34 +0800`；改 workflow 合进中间提交；改文档 amend 最后一个提交并保留其 AuthorDate。作者使用 Cursor Agent。
- 仅修改 `.cursor/Dockerfile`、`.cursor/Dockerfile.luckfox_pico`、`.cursor/environment.json`、`.cursor/install.sh`、`AGENTS.md`、`.github/workflows/build-luckfox-pico-firmware.yml`、本 plan、2026-09-14 spec、2026-07-13 spec 中已对齐的 `install` 指针、2026-07-17 spec 中 `build-firmware` 的 `container.options`，以及 2026-08-30 spec §2.3 命令字符串、§3.2 用户表与 Q8 中同一组 `install`/`start` 字符串。不改 `./build.sh`、板上固件、`.cursor/start.sh` 正文、08-30 plan、07-17 plan、Tailscale policy/ACL。
- 禁止 JSON `"user"`；禁止 `sudo -H`；禁止去掉 `-E`；禁止 `chpasswd` / 明文用户密码；禁止 chmod `/root`；禁止硬编码 `/home/ubuntu` 作为 grilling 路径。
- 备选 Dockerfile 保留 `id -u ubuntu >/dev/null 2>&1 || useradd --create-home --shell /bin/bash ubuntu`，不锁 `-u 1000`。`USER` 必须在 docker build 的全部 `RUN` 之后；JSON 的 `sudo -n -E` 不覆盖构建期 `RUN`。固件 CI `build-firmware` 必须 `options: --user 0`（[Dockerfile support → USER](https://docs.github.com/en/actions/reference/workflows-and-actions/dockerfile-support#user)；与 `--user root` 的区别见 Spec §2.3）；禁止把镜像 `ubuntu` 改成 uid 1001。
...
| `.github/workflows/build-luckfox-pico-firmware.yml` | `build-firmware.container.options` 为 `--user 0`，覆盖镜像 `USER ubuntu` |
...
| `docs/superpowers/specs/2026-07-17-luckfox-github-actions-ci-design.md` | F2 / §4.2 / §6：`container.options: --user 0` |
...
- Modify: `.github/workflows/build-luckfox-pico-firmware.yml`（`build-firmware.container.options: --user 0`）
...
- [x] **Step 5:** `build-firmware` 加 `options: --user 0`
- [x] **Step 6:** 实现提交（仅 `.cursor` 运行时文件；AuthorDate `2026-09-11 01:30:34 +0800`）
- [x] **Step 7:** 固件 CI 提交（仅 `.github/workflows/build-luckfox-pico-firmware.yml`，居中）

### Task 2: F7 / F8 文档指针（已完成）

**Files:**

- Modify: `AGENTS.md:17`
- Modify: `docs/superpowers/specs/2026-08-30-luckfox-cloudagent-tailscale-design.md`（§2.3 两处命令字符串、§3.2 用户表、Q8 中同一组字符串）
- Modify: `docs/superpowers/specs/2026-07-17-luckfox-github-actions-ci-design.md`（F2 / §4.2 / §6：`container.options: --user 0`）

2026-07-13 spec 的 `install` 字符串已是 `sudo -n -E bash .cursor/install.sh`，不要再改。不在 `AGENTS.md` 写端口表、不抄 `install`/`start` 命令、不链 spec。

**Interfaces:** 消费 Spec F7 / F8 / §2.2 / §2.3 / §2.4；不改运行时脚本。
...
- [x] **Step 5: 改 07-17 spec 的 CI 容器用户**

将 F2、§4.2「两个 job」条、§6 `build-firmware` 条写明 `container.options: --user 0`，并链 [Dockerfile support → USER](https://docs.github.com/en/actions/reference/workflows-and-actions/dockerfile-support#user)。与 `--user root` 的区别指向 09-14 spec §2.3。不改 07-17 plan 的历史 Task YAML。
...
grep -n 'options: --user 0' .github/workflows/build-luckfox-pico-firmware.yml
grep -n 'options: --user 0' docs/superpowers/specs/2026-07-17-luckfox-github-actions-ci-design.md
if grep -nE 'options: --user root' .github/workflows/build-luckfox-pico-firmware.yml; then exit 1; fi
```

Expected: python 退出 0；`AGENTS.md` 命中 `默认会话用户为 \`ubuntu\``；四个 `if grep` 不命中（不进入 `exit 1`）；workflow 与 07-17 spec 命中 `options: --user 0`；workflow 的 `options:` 行不是 `--user root`。

- [x] **Step 7: 把 F7 / F8 文本合进对应提交**

`AGENTS.md` 与全部 superpowers 文档（07-13 / 07-17 / 08-30 / 09-14 spec 与本 plan）合进最后一个提交，保留其 AuthorDate。`.github/workflows/build-luckfox-pico-firmware.yml` 单独居中提交。实现提交不含 `AGENTS.md`。

### Task 3: 验收（已完成）

**Files:** 验证 `.cursor/*` 与 `.github/workflows/build-luckfox-pico-firmware.yml`；通过标准见 Spec §5；实测值回填本 plan「最终验证证据」。

**Interfaces:** 消费 Task 1 的 draft Build；V8 在目标 Agent 上不以 sudo 调用 `./build.sh`。

- [x] **Step 1: 静态 V3 / V4 / V9**（仓库文件已满足：JSON 无 `user`、末指令 `USER ubuntu`、备选有 `useradd`、`build-firmware` 含 `options: --user 0`）
...
| 交付物 | 状态 |
| --- | --- |
| Task 1 运行时（Dockerfile `USER ubuntu`、JSON `sudo -n -E`、grilling `chown`、CI `--user 0`） | ✅ |
| Task 2 F7 / F8（`AGENTS.md` 会话用户句、08-30 spec 指针、07-17 spec `container.options`） | ✅ |
| Task 3 验收 V1–V9 | ✅ |
| spec + plan | ✅ |

**整体结论**：Cloud Agent 交互默认用户为 `ubuntu`；`install`/`start` 仍以 root 执行特权步骤；固件 CI `build-firmware` 以 `--user 0` 覆盖镜像 `USER`。活动与备选镜像均已在 draft Build 上通过 Spec §5 的 V1–V8（含负向门禁）；V9 为仓库静态核对。规格已实施。
...
- `build-firmware.container` 含 `options: --user 0`
- `options:` 行不是 `--user root`
- 固件 workflow 仅 `push:dev` / `schedule` / `workflow_dispatch`；本 feature 分支无 checks 是预期。合入 `dev` 后首次 `push(dev)` 才跑三组 `build-firmware`
...
- 活动 `.cursor/Dockerfile` 同时是固件 CI 镜像。镜像保留 `USER ubuntu`；`build-firmware` 用 `--user 0` 覆盖，不把 `ubuntu` 改成 uid 1001。`--user 0` 按数字指定 uid 0；`--user root` 按 passwd 名解析。依据 [Dockerfile support → USER](https://docs.github.com/en/actions/reference/workflows-and-actions/dockerfile-support#user) 与 Spec §2.3。
- 固件 CI workflow 单独居中提交；`AGENTS.md` 与全部 superpowers 文档在最后一个提交。相对 `origin/dev` 共三个提交。
